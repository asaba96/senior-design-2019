...
from mbz2020_common.msg import MotionPointStamped, MotionPointStampedArray
...
class LinearMotionProfileGenerator(object):
    ...
    def __init__(self, start_motion_point):
        ...
        try:
            ...
            rospy.logerr(
                'TaskConfig: Could not lookup a parameter for linear motion profile generator'
            )
            raise
        ...
        # if we have only received velocity commands that do not set the initial position
        # then everything is relative to 0,0,0
    ...
    # set whole trajectory of points
    def set_trajectory(self, plan):
    ...
    # set a single motion point
    def set_motion_point(self, motion_point):
        if len(self._last_motion_plan.motion_points) == 0:
            self._last_motion_plan.motion_points = [motion_point]
        else:
            new_plan = MotionPointStampedArray()
            new_point_stamp = motion_point.header.stamp
            ...
            new_plan.motion_points.append(motion_point)
            self._last_motion_plan = new_plan

        self._is_relative = False

    # returns the current motion plan
    def get_motion_plan(self, time=None):
        if time is None:
            return self._last_motion_plan
        else:
            # TODO: fix to return only specified length of plan
            return self._last_motion_plan
    ...
    # returns the expected motion point at a given time
    ...
    def get_velocity_plan(self, velocity_command):
        # Get the starting motion point for the time that the velocity is desired
        ...
        if acceleration > self._MAX_TARGET_ACCEL:
            ...
            rospy.logerr('TaskConfig: linear motion profile generator \
